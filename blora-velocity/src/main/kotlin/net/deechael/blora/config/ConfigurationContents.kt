...
data class Authorization(
    ...
    @TomlComment("离线登录的玩家名称的前缀")
    val offlinePrefix: String = "*",
    @TomlComment("离线登录的玩家名称的后缀")
    val offlineSuffix: String = "",
...
@Serializable
data class Security(
    @TomlComment("当玩家未验证时允许使用的命令（已过时，）")
    val unauthorizedAllowedCommands: List<String> = listOf(
        "login",
        "register",
        "l",
        "reg",
    ...
    @TomlComment("（暂未支持）受保护的指令，玩家执行这些指令时将不会输出到控制台，本插件提供的指令默认被保护")
    val protectedCommands: List<String> = listOf(
        "msg",
        "whisper",
        "w",
        "tell",
    ...
    @TomlComment("定义一系列弱密码，用户无法使用这些密码")
    ...
    fun buildDataSource(): HikariDataSource {
        return HikariDataSource(
            HikariConfig().apply {
                ...
                this@apply.poolName = "Bloret Login MariaDB Connection Pool"
