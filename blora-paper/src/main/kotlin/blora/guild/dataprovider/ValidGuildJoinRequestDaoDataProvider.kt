...
class ValidGuildJoinRequestDaoDataProvider(
    ...
    private val removedFilter: MutableList<GuildJoinRequestDao> = mutableListOf()
    ...
    private val hook: (EntityChange) -> Unit = { change ->
        if (change.entityClass == GuildJoinRequestDao) {
            val entity = change.toEntity(GuildJoinRequestDao)
